...
class Window extends EventHandler
{
   ...
   */
                   ...
                   ?inSize:Size, ?inStyle:Int )
   {
      ...
      var handle = wx_window_create(arr);
   ...
   private function get_size() : Size { return wx_window_get_size(wxHandle); }
   private function set_size(inSize:Size) : Size
   {
      wx_window_set_size(wxHandle, inSize);
      return inSize;
   }

   private function get_sizer() : Sizer { return wx_window_get_sizer(wxHandle); }
   private function set_sizer(inSizer:Sizer) : Sizer
   {
      wx_window_set_sizer(wxHandle, inSizer.wxGetHandle());
      return inSizer;
   }

   private function get_clientSize() : Size { return wx_window_get_client_size(wxHandle); }
   private function set_clientSize(inSize:Size) : Size
   {
      wx_window_set_client_size(wxHandle, inSize.width, inSize.height);
      return inSize;
   }

   private function get_position() : Position { return wx_window_get_position(wxHandle); }
   private function set_position(inPos:Position) : Position
   {
      wx_window_set_position(wxHandle, inPos);
      return inPos;
   }

   private function get_shown() : Bool { return wx_window_get_shown(wxHandle); }
   private function set_shown(inShow:Bool = true) : Bool
   {
      wx_window_set_shown(wxHandle, inShow);
      return inShow;
   }

   private function get_backgroundColour() : Int { return wx_window_get_bg_colour(wxHandle); }
   private function set_backgroundColour(inColour:Int) : Int
   {
      wx_window_set_bg_colour(wxHandle, inColour);
      return inColour;
   }

   private function get_name() : String { return wx_window_get_name(wxHandle); }
   private function set_name(inName:String) : String
   {
