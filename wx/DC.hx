...
class DC
{
   ...
   private function set_pen(inPen:Pen) : Pen
   {
      wx_dc_set_pen(wxHandle,inPen.wxGetHandle());
      return inPen;
   }
   private function set_brush(inBrush:Brush) : Brush
   {
      wx_dc_set_brush(wxHandle,inBrush.wxGetHandle());
      return inBrush;
   }
   private function set_background(inBrush:Brush) : Brush
   {
      wx_dc_set_background(wxHandle,inBrush.wxGetHandle());
      return inBrush;
   }
   private function set_font(inFont:Font) : Font
   {
